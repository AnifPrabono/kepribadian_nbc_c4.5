...
function can_access_menu($menu){
    if($_SESSION['kepribadian_nbc_c4.5_level']==2 & ($menu=='klasifikasi')){
        return true;
    }
    if($_SESSION['kepribadian_nbc_c4.5_level']==1 & (($menu=='data_latih')
            || ($menu=='uji_akurasi') || ($menu=='laporan_hasil') || ($menu=='data_siswa') || ($menu=='data_soal') ||
            ($menu!='mining') || ($menu!='proses_mining') || ($menu=='pohon_keputusan') || ($menu=='uji_rule') || ($menu=='hitung_akurasi') )){
        return true;
    }
    return false;
}

// function can_access_menu_c45($menu){
//     if($_SESSION['kepribadian_nbc_c4.5_level']==2 & ($menu=='klasifikasi_c45')){// || $menu=='view_rule')){
//         return true;
//     }
//     if($_SESSION['kepribadian_nbc_c4.5_level']==1 & (($menu!='mining')
//             || ($menu=='pohon_keputusan') || ($menu=='laporan_hasil') || ($menu=='data_siswa') || $menu=='data_soal')){
//         return true;
//     }
//     return false;
// }
...
function sudah_klasifikasi_c45($db_object, $id_siswa){
    $sql = "SELECT COUNT(*) FROM jawaban_kuisioner_c45 "
            . " WHERE id_siswa = ".$id_siswa;
    $result = $db_object->db_query($sql);
    $row = $db_object->db_fetch_array($result);
    return $row[0]>0;
...
function get_hasil_klasifikasi_c45($db_object, $id_siswa){
    $sql = "SELECT * FROM data_hasil_klasifikasi_c45 WHERE id_siswa = ".$id_siswa;
    $result = $db_object->db_query($sql);
    $row = $db_object->db_fetch_array($result);
    return $row;
}
